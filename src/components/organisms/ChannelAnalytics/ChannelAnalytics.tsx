...
export const ChannelAnalytics: React.FC<ChannelAnalyticsProps> = ({ className }) => {
    ...
    let timeBasedData: {[key: string]: {[platform: string]: SocialMetric[]}} = {
      ...
      'Custom': {
        ...
        'facebook': [
          ...
          { label: 'Engagements', value: '13.2K', change: '+10%', changeLabel: 'in selected period', isPositive: true }
        ],
        'linkedin': [
          { label: 'Connections', value: '8.2K', change: '+4.1%', changeLabel: 'in selected period', isPositive: true },
          { label: 'Posts', value: '12', change: '+12', changeLabel: 'in selected period', isPositive: true },
          { label: 'Queued Posts', value: '5', change: '+2', changeLabel: 'since period start', isPositive: true },
          { label: 'Engagements', value: '3.7K', change: '+9%', changeLabel: 'in selected period', isPositive: true }
        ],
        'pinterest': [
          { label: 'Followers', value: '4.0K', change: '+6.5%', changeLabel: 'in selected period', isPositive: true },
          { label: 'Pins', value: '45', change: '+45', changeLabel: 'in selected period', isPositive: true },
          { label: 'Queued Pins', value: '10', change: '+4', changeLabel: 'since period start', isPositive: true },
          { label: 'Engagements', value: '2.1K', change: '+18%', changeLabel: 'in selected period', isPositive: true }
        ]
      }
    };
    
    if (timeFilter === 'Custom' && customRange) {
      // In a real app, you would fetch data for the custom date range from the API
      const customData = timeBasedData['Custom'][account.platform];
      return processCustomRangeMetrics(account, customRange, customData);
    }
    
    // Return the metrics for the selected time filter and platform
  ...
  const handleDateRangeSelect = (startDate: Date, endDate: Date) => {
    ...
    setCustomDateRange({ startDate: validStartDate, endDate: validEndDate });
    setShowDatePicker(false);
    fetchMetrics('Custom', { startDate: validStartDate, endDate: validEndDate });
  };
  ...
  const processCustomRangeMetrics = (account: SocialAccount, range: TimeRange, customData: SocialMetric[]) => {
    ...
    // Update the change labels to reflect the custom date range
    return customData.map(metric => ({
      ...metric,
      changeLabel: `over ${days} days`
    }));
  };
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (datePickerRef.current && !datePickerRef.current.contains(event.target as Node) && event.target instanceof Element && !event.target.closest('.custom-date-filter')) {
        setShowDatePicker(false);
      }
    }
